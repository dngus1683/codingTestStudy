...
### Complexity
+ 시간 복잡도: O(n^2), O(n) (최선: 이미 정렬이 되었을 경우를 대비해 종료조건을 추가한 경우)
+ 공간 복잡도: O(1) (최선), O(n) (최악)
...
## 삽입 정렬
### Algorithm
1. 배열을 정렬된 부분(**배열 앞**)과 정렬되지 않은 부분(**배열 뒤**)으로 나눈다.
2. 정렬되지 않은 부분의 첫 번째 원소를 정렬된 부분에 삽입한다. (**시작은 두번째 원소부터**)
...
### Complexity
+ 시간 복잡도: O(n) (최선: 이미 정렬된 경우), O(n^2) (평균, 최악)
+ 공간 복잡도: O(1) (최선), O(n) (최악)
...
2. 피벗을 기준으로 작은 원소는 왼쪽, 큰 원소는 오른쪽으로 분할한다.
3. 분할된 부분 배열에 대해 재귀적으로 퀵 정렬을 수행한다.
...
### Complexity
+ 시간 복잡도: O(n log n) (최선, 평균), O(n^2) (최악)
+ 공간 복잡도: O(log n) (재귀 호출 스택), O(n log n) (최악)
...
2. 최대 힙에서 최댓값(루트)을 제거하고 배열의 마지막 원소와 교환한다.
3. 배열의 크기를 줄이고 다시 최대 힙을 구성한다.
4. 위 과정을 반복하여 전체 배열이 정렬될 때까지 진행한다.
...
## 기수 정렬
### Algorithm
1. 가장 작은 자리수부터 가장 큰 자리수까지 순서대로 정렬한다.
2. 각 자리수에 대해 숫자를 세는 계수 정렬을 사용한다.
...
## 계수 정렬
### Algorithm
1. 배열의 각 요소가 몇 번 등장하는지 세고, 해당 숫자의 등장 횟수를 기록한다.
2. 등장 횟수를 바탕으로 정렬된 배열을 생성한다.
